...
float quantize(float value, float levels) {
    ...
    return step * floor(value / step) + step * 0.5;
}
...
void main() {
    ...
    float diff = quantize(max(dotNormLight, 0.0), toonLevels);
    ...
    float spec = 0.0;
    if (dotNormLight > 0.0) {
        spec = quantize(pow(max(dot(viewDir, reflectDir), 0.0), material.shininess), toonLevels);
    }
